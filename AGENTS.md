...
- Require schema version 2: shared checkpoint, version, timestamp and count fields use native signed 64-bit integers. Object-state IDs are `<kind>:<20-digit source checkpoint>:<immutable object-version ID>`; NAVI value IDs use the `emode` kind prefix. The source checkpoint, never the later materialization checkpoint, determines replay identity. This schema needs a fresh processor version and backfill; do not read version-1 rows as version 2.
- Read immutable object-version rows through `PortfolioObjectState_raw`. Bound scans by kind/source-checkpoint ID ranges, select the largest immutable ID per object before fetching JSON payloads, and deduplicate quote IDs to absorb deterministic replay writes. Keep snapshot/value/metadata entity aliases on their latest views. Completion counts cover the selected interval; they are not a cumulative proof of every earlier interval. Keep the host freshness gate while a processor backfills.
